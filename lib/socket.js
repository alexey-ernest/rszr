...
module.exports = function (server) {
   ...
   */
  function onSocketComplete(socket, base64) {
    ...
    s3.putObject(params, function (err) {
      if (err) {
        debug('An error occured while uploading resized image to ' + msgData.targetUri + ': ' + err);
  ...
  function downloadImage(uri, fn) {
     ...
     * @param      {Function}              fn        Callback.
     */
    function handleImageResponse(response, fn) {
      var err;
    ...
    request
      ...
      .on('response', function(response) {
        handleImageResponse(response, function (err, buf) {
  ...
  function onResizeMessage(msg) {
     ...
     */
    function emitProcessEvent(socket, buffer, contentType) {
    ...
    downloadImage(data.uri, function (err, buf, contentType) {
      ...
      emitProcessEvent(freeSocket, buf, contentType);
    });
  }
